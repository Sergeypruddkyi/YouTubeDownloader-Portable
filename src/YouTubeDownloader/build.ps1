...
$icon = Join-Path $root 'assets\YouTubeDownloader.ico'
if (-not (Test-Path -LiteralPath $icon)) { throw 'assets\YouTubeDownloader.ico не найден' }
$outExe = Join-Path $out 'YouTubeDownloader.exe'
$files = Get-ChildItem -LiteralPath $src -Filter '*.cs' | ForEach-Object { $_.FullName }
& $csc /nologo /target:winexe /platform:anycpu /optimize+ /codepage:65001 /win32manifest:$manifest /win32icon:$icon /out:$outExe /r:System.dll /r:System.Core.dll /r:System.Windows.Forms.dll /r:System.Drawing.dll /r:Microsoft.CSharp.dll @files
